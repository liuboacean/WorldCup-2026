  ...
  async function init() {
    ...
    if (dom.todayBtn) {
      dom.todayBtn.addEventListener('click', () => {
        ...
        // Try exact today, or fall back to next future match day
        const dateSections = dom.matchesContainer.querySelectorAll('.date-section');
        let target = null;
        for (const sec of dateSections) {
          const label = sec.querySelector('.date-label');
          if (label && label.textContent.includes(todayStr)) { target = sec; break; }
        }
    ...
    if (dom.liveBtn) {
      dom.liveBtn.addEventListener('click', () => {
        ...
        if (firstLive) {
          ...
          setTimeout(() => { firstLive.style.boxShadow = ''; }, 3000);
        } else {
          const liveCount = document.getElementById('liveCountText');
          dom.todayBtn.click();
        }
      });
    }
